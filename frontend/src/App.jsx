...
function Sparkline({ symbol, up }) {
  ...
  const points = Array.from({ length: 8 }, (_, i) => {
    ...
    return Math.max(
      2,
      Math.min(38, 20 - trend - variance * 0.4)
    );
  });
...
export default function App() {
  ...
  const refreshAttention = useCallback(async () => {
    ...
    for (const item of watchlist.items) {
      try {
        results[item.symbol] = await getAttention(
          USER_ID,
          item.symbol
        );
      } catch (err) {
        console.error(
          `Failed to fetch attention for ${item.symbol}:`,
          err
        );
      }
    }
  ...
  const handleAdd = async (e) => {
    ...
    try {
      ...
      setError(
        "Failed to add symbol — check console for details."
      );
    }
  };
  ...
  const highImpact = withData.filter(
    (i) => i.data.attention === "high"
  );
  ...
  const filtered = meaningful.filter((i) => {
    ...
    if (filter === "high") {
      return i.data.attention === "high";
    }

    if (filter === "gainers") {
      return (i.data.quote?.pct_change ?? 0) >= 0;
    }
        ...
        <main className="content">
          {activeNav === "Watchlists" ? (
            ...
            <div className="placeholder-view">
              ...
              <p>
                This view isn't built yet — happy to do it next.
              </p>
            </div>
          ) : (
            <>
              ...
              {items.length === 0 ? (
                <div className="empty-state">
                  ...
                  <p>
                    Add a symbol above to start tracking
                    meaningful changes.
                  </p>
                </div>
              ) : filtered.length === 0 ? (
                <div className="empty-state">
                  <h3>
                    Nothing major changed since you last
                    checked
                  </h3>
                ...
                filtered.map(({ symbol, data }) => {
                  const meta =
                    LEVEL_META[data.attention] ||
                    LEVEL_META.routine;

                  const change =
                    data.quote?.pct_change;
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="change-impact">
                        {meta.impact && (
                          <span
                            className={`impact-badge ${meta.badgeClass}`}
                          >
                            {meta.impact}
                          </span>
                        )}
